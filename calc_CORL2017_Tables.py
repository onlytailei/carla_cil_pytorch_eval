...
from carla.driving_benchmark.experiment_suites import CoRL2017, VrgTransferSuite
...
if (__name__ == '__main__'):
    ...
    # TODO: add time instead on zero
    logging.info('sarting the calculations %s', "0")

    # experiment_suite = CoRL2017("Town01")
    experiment_suite = VrgTransferSuite("Town01", [1, ])
    ...
    tasksSuccessRate = {0: 'Straight',
                        1: 'One Turn',
                        2: 'Navigation',
                        3: 'Nav. Dynamic'}

    tasksInfractions = {0: 'Opposite Lane',
                        1: 'Sidewalk',
                        2: 'Collision-static',
                        3: 'Collision-car',
    ...
    states = {}
    allpath = {}
    for i, path_ in enumerate(allDir):
        # print(path_.split('/')[-1][:-24])
        if path_[-3:] != 'tml' and path_[-3:] != 'Tex':
            states[i] = path_.split('/')[-1][:-24]
            allpath[i] = path_
    # extractedPath = allDir[0].split('/')[-1]
    # print(extractedPath)
    # logging.info(
    #     'Please make sure all the subdirectory of %s start with %s',
    #     args.path,
    #     extractedPath)

    # In CoRL-2017 paper, infraction are only computed on the fourth task - "Navigation with dynamic obstacles".
    dataSuccessRate = np.zeros((len(tasksSuccessRate),
                                len(states)))  # hold the whole table 1 data
    dataInfractions = np.zeros((len(tasksSuccessRate),
                                len(tasksInfractions),
                                len(states)))  # hold the whole table 2 data
    dataNumInfractions = np.zeros((len(tasksSuccessRate),
                                   len(tasksInfractions),
                                   len(states)))  # hold the whole table 3 data
    dataSuccessRateSTD = np.zeros((len(tasksSuccessRate),
                                   len(states)))  # hold the whole table 1 std data
    dataInfractionsSTD = np.zeros((len(tasksSuccessRate),
                                   len(tasksInfractions),
                                   len(states)))  # hold the whole table 2 std data
    dataNumInfractionsSTD = np.zeros((len(tasksSuccessRate),
                                      len(tasksInfractions),
                                      len(states)))  # hold the whole table 3 std data
    ...
    for (sIdx, state), idx in zip(enumerate(states), allpath):
        # print(path)
        logging.info('State: %s, %s', state, allpath[idx])
        path = allpath[idx]
        ...
        # episodes_fully_completed
        values = metrics_summary[metrics_to_average[0]]
        if(table1Flag):
        ...
        if(table2Flag):
            ...
            for metricIdx, metric in enumerate(infraction_metrics):
                ...
                for items_metric, items_driven in zip(
                                                      values.items(),
                                                      values_driven.items()
                                                      ):
                ...
                for i in range(number_of_tasks):
                    ...
                    if metric_sum_values[i] == 0:
                        dataListTable2[metricIdx][i].append(
